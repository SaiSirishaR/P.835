...
g = open('diffs_file.txt','w')


####### Modified by Sai Sirisha Rallabandi on 26 Jan 2021 ###############################
...
#    correct_tps = 0
    ...
    try:        
        ...
        print(tp_itemcode)
        suffix = tp_itemcode.lower()
        print(suffix)

        for q_name in question_names:
          
            print("qname is", q_name)
            print("normal", int(row[f'answer.{q_name}_{suffix}']), "tp", int(row[f'answer.{q_name}_tp']), "diff is", int(row[f'answer.{q_name}_{suffix}']) - int(row[f'answer.{q_name}_tp']))
            g.write(str(int(row[f'answer.{q_name}_{suffix}']) - int(row[f'answer.{q_name}_tp'])) + '\n')
            if int(row[f'answer.{q_name}_{suffix}']) - int(row[f'answer.{q_name}_tp']) < 15 and int(row[f'answer.{q_name}_{suffix}']) - int(row[f'answer.{q_name}_tp']) >- 15:
                    print("accepted diff is", int(row[f'answer.{q_name}_{suffix}']) - int(row[f'answer.{q_name}_tp']))
                    correct_tps += 1
            else:
                 pass
#        return correct_tps
...
def check_variance(row):
    ...
    """
    r = []
    attr_list = ['ACCE', 'ARRO', 'CONF', 'ENER', 'ENTH', 'EXTR', 'FRIE', 'KIND', 'LIKE', 'PLEA', 'RELA', 'RELI', 'RESP', 'SKIL', 'SYMP', 'TP']
    for q_name in question_names:
        for attr in attr_list:       
            attr = attr.lower()

            print("in variance qname is", q_name, row[f'answer.{q_name}_{attr}'])
...
#            continue
            try:
               print("in variance", int(row[f'answer.{q_name}_{attr}']))
               r.append(int(row[f'answer.{q_name}_{attr}']))
            except:
              pass
    try:
        v = statistics.variance(r)
        print("variance of the participant rating is", v)
        return v
    except:
        pass
    return -1
...
def data_cleaning(filename, method):
   ...
   with open(filename, encoding="utf8") as csvfile:
    ...
    for row in reader:
        ...
        # step 4. check tps
        ctps = 0
        d['correct_tps'] = check_tps(row, method, ctps)
        print("correct tps are", d['correct_tps'])
...
def transform(test_method, sessions, agrregate_on_condition):
    ...
    for session in sessions:
        ...
        for question in question_names:
            ...
            if session[f'answer.{question}_tp']: ####### Modified by Sai Sirisha Rallabandi on 26 Jan 2021 
                continue
            # is it a gold clips
